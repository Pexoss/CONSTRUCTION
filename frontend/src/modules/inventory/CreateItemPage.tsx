...
const CreateItemPage: React.FC = () => {
  ...
  const handlePricingInputBlur = (field: string, value: string) => {
    ...
    if (errors[`pricing.${field}`]) {
      setErrors((prev) => ({ ...prev, [`pricing.${field}`]: "" }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend: CreateItemData = {
        ...formData,
        pricing: {
          ...formData.pricing,
      ...
      if (formData.trackingType === "unit") {
        ...
        if (!formData.quantity.total || formData.quantity.total <= 0) {
          setErrors({
            quantity:
              "A quantidade total deve ser maior que zero para itens quantitativos",
          });
          return;
        }
      ...
      if (dataToSend.depreciation?.purchaseDate) {
        dataToSend.depreciation.purchaseDate = new Date(
                ...
                <div className="space-y-4">
                  <div className="flex items-start">
                    <input
                      ...
                      onChange={(e) => {
                        setFormData((prev) => ({
                          ...prev,
                          trackingType: e.target.value as "quantity" | "unit",
                        }));
                        setUnits([]);
                      }}
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Ex: Furadeira F421, Betoneira B013
                      </p>
                    </div>
                  </div>
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                const newUnits = [...units];
                                newUnits[index].status = e.target.value as
                                  | "available"
                                  | "rented"
                                  | "maintenance"
                                  | "damaged";
