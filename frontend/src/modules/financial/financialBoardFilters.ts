...
export function billingMatchesBoardFilters(billing: any, f: FinancialBoardUrlFilters): boolean {
  const normalizedItemFilter = foldAccents(f.itemText.trim());
  const normalizedObraFilter = foldAccents(f.obraText.trim());
  ...
  const itemNames = foldAccents(
    (billing.items || [])
      .map((item: any) => String(item?.itemId?.name || item?.itemId || "").trim())
      .join(" "),
  );
  ...
  const obraHaystack = foldAccents(
    `${workAddress?.workName || ""} ${workAddress?.city || ""} ${workAddress?.state || ""}`,
  );
  const matchesObra = !normalizedObraFilter || obraHaystack.includes(normalizedObraFilter);
