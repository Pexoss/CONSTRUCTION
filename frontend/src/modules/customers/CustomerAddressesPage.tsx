...
const CustomerAddressesPage: React.FC = () => {
  ...
  const [zipLookupError, setZipLookupError] = useState<Record<number, string>>({});
  ...
  const normalizeZipCode = (value: string): string => value.replace(/\D/g, "");
  ...
  const fetchAddressByZipCode = async (index: number) => {
    const zipCode = normalizeZipCode(addresses[index]?.zipCode || "");

    if (zipCode.length !== 8) {
      setZipLookupError((prev) => ({
        ...prev,
        [index]: "Digite um CEP com 8 dígitos para buscar.",
      }));
      return;
    }

    setZipLookupLoading((prev) => ({ ...prev, [index]: true }));
    setZipLookupError((prev) => ({ ...prev, [index]: "" }));
    setZipLookupSuccess((prev) => ({ ...prev, [index]: "" }));
    ...
    try {
      ...
      newAddresses[index] = {
        ...
        neighborhood: data.bairro || currentAddress.neighborhood || "",
        city: data.localidade || currentAddress.city || "",
        state: (data.uf || currentAddress.state || "").toUpperCase(),
      };

      setAddresses(newAddresses);
      setZipLookupSuccess((prev) => ({
        ...prev,
        [index]: "Endereço preenchido automaticamente pelo CEP.",
      }));
    } catch (error) {
      setZipLookupError((prev) => ({
        ...prev,
        [index]: "Não foi possível consultar o CEP agora.",
      }));
    } finally {
      setZipLookupLoading((prev) => ({ ...prev, [index]: false }));
    }
  };
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onBlur={() => {
                            if (normalizeZipCode(address.zipCode || "").length === 8) {
                              fetchAddressByZipCode(index);
                            }
                          }}
                          disabled={isLoading || zipLookupLoading[index]}
                          className="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                        <button
                          type="button"
                          onClick={() => fetchAddressByZipCode(index)}
                          disabled={isLoading || zipLookupLoading[index]}
                          className="px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-xs font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                        >
                          {zipLookupLoading[index] ? "Buscando..." : "Buscar CEP"}
                        </button>
                      </div>
                      {zipLookupError[index] && (
                        <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">
                          {zipLookupError[index]}
                        </p>
                      )}
                      {zipLookupSuccess[index] && !zipLookupError[index] && (
                        <p className="mt-1 text-xs text-green-600 dark:text-green-400 inline-flex items-center gap-1">
                          <svg
